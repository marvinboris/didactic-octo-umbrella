    ...
    if (!dropdown) content = <>
        ...
        const itemEls = items.map(({ link = '', text }) => (
            <li className="nav-item text-400" key={text}>
                <NavLink onClick={sideDrawerToggle} exact className={`nav-link d-sm-none text-${dark ? "white" : "secondary"}`} activeClassName="text-700" to={path + link}><span className="position-relative text-400" style={{ left: -8 }}>-</span> {text}</NavLink>
                <NavLink exact className={`nav-link d-none d-sm-block text-${dark ? "white" : "secondary"}`} activeClassName="text-700" to={path + link}><span className="position-relative text-400" style={{ left: -8 }}>-</span> {text}</NavLink>
            </li>
        ));

        content = (
            <div>
                <Button color="link" onClick={toggle} className={`SideDrawerItem nav-link d-block w-100 rounded-0 border-0 p-0 text-left text-${dark ? "white" : "blue"} position-relative ${match ? 'active' : ''}`} style={{ boxShadow: 'none' }}>
                    <div className="py-2 px-3 text-400 rounded-4">
                        <i className={"fas fa-" + icon + " fa-fw mr-3"} />
