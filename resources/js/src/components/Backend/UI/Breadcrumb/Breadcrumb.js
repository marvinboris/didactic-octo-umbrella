    ...
    render() {
        ...
        return (
            <B className="d-none d-sm-flex align-items-center" color={dark ? "light" : "secondary"} listClassName="bg-transparent rounded-0 justify-content-end" style={{ top: '50%', right: 0, transform: 'translateY(-10px)', position: 'absolute', zIndex: 1000 }}>
                <BreadcrumbItem><NavLink className={`text-green text-decoration-none`} to="/">{icon && <i className={"fas fa-" + icon + " mr-1"} />}<span className='text-500 text-green-50'>{home}</span></NavLink></BreadcrumbItem>
                {itemsComponent}
                <BreadcrumbItem color='green' className={`text-decoration-none`} active>{main}</BreadcrumbItem>
            </B>
        );
    }
}
